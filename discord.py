    ...
    if last_session.is_new() and last_session.handshakes > 0:
        ...
        try:
                ...
                'embeds': [
                    {
                        ...
                        'footer': {
                            'text': f'Pwnagotchi v{pwnagotchi.version} - Discord Plugin v{__version__}'
                        },
                        'image': {
            ...
            with open(picture,'rb') as image:
                # OPTIONS['webhook_url'] becomes self.options['webhook_url'] in new version
                requests.post(OPTIONS['webhook_url'], files={'image': image, 'payload_json': (None, json.dumps(data))})
